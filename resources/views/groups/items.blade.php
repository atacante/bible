            ...
            <div class="my-inner-groups-item">
                ...
                <div title="{!! Auth::user() && Auth::user()->isPremium() || in_array($group->id,$content['joinedGroupsKeys'])?'':'Premium Feature' !!} {!! Auth::user() && Auth::user()->isBanned('group',$group->id)?'You were banned from being part of this group':'' !!}" class="btns-group1">
                    @if((isset($dataKey) && $dataKey == 'myGroups') || /*in_array($group->id,$content['joinedGroupsKeys']) ||*/ (Auth::user() && $group->owner_id == Auth::user()->id))
                        <a title="Update group" href="{!! url('/groups/update/'.$group->id,[]) !!}" class="btn1-icon"><i class="bs-settings cu-btn-ic"></i></a>
                        <a title="Delete group" href="{!! url('/groups/delete/'.$group->id,[]) !!}" class="btn2-icon j-remove-group" data-toggle="modal" data-target="#confirm-delete" data-header="Delete Confirmation" data-confirm="Are you sure you want to delete this item?"><i class="cu-btn-ic bs-remove"></i></a>
                    @else
                        @if(isset($dataKey) && $dataKey == 'groupsRequested')
                            <a title="Accept group" href="{!! url('/groups/accept-request/'.$group->id.'/'.Auth::user()->id,[]) !!}" class="btn1-icon j-accept-request"><i class="bs-checkmark cu-btn-ic"></i></a>
                            <a title="Reject group" href="{!! url('/groups/cancel-request/'.$group->id.'/'.Auth::user()->id,[]) !!}" class="btn2-icon j-cancel-request" data-toggle="modal" data-target="#cancel-request-sm" data-header="Reject Request" data-confirm="Are you sure you want to reject this request?"><i class="cu-btn-ic bs-close"></i></a>
                        @else
                            @if(Auth::check() && Auth::user()->is('user'))
                            <a href="{!! url('/groups/cancel-request/'.$group->id.'/'.Auth::user()->id,[]) !!}" class="btn2 cu9-btn1 j-cancel-request {{((!isset($dataKey) || (isset($dataKey) && ($dataKey == 'groups' || $dataKey == 'myGroupsRequests'))) && Auth::check() && in_array($group->id,Auth::user()->myGroupsRequests->modelKeys()))?'':'hidden'}}" data-toggle="modal" data-target="#cancel-request-sm" data-header="Cancel Request" data-confirm="Are you sure you want to cancel this request?">Cancel <br> Request</a>
                            <a href="{!! url('/groups/leave-group/'.$group->id,[]) !!}" class="btn2 cu5-btn1 j-leave-group {!! in_array($group->id,$content['joinedGroupsKeys'])?'':'hidden' !!}">Leave</a>
                            @endif
                            <a href="{!! url('/groups/join-group/'.$group->id,[]) !!}" class="btn1 cu5-btn1 j-join-group {!! in_array($group->id,$content['joinedGroupsKeys']) || (Auth::check() && in_array($group->id,Auth::user()->myGroupsRequests->modelKeys()))?'hidden':'' !!} {!! Auth::user() && Auth::user()->isPremium()?'':'disabled' !!} {!! Auth::user() && Auth::user()->isBanned('group',$group->id)?'disabled':'' !!}">Join</a>
                        @endif
                    @endif
                </div>
            </div>
...
@else
    <div class="text-center" style="padding: 10px; line-height: 23px;">
        ...
        <a href="{!! url('/groups?type=all',[]) !!}" class="btn2-kit" style="margin: 10px;"><i class="bs-search cu-search2"></i>Discover Groups</a>
        @elseif(isset($dataKey) && $dataKey == 'myGroups')
        You haven’t got any groups yet.</br>
        Click at the button below to create new.</br>
        <a href="{!! url('/groups/create',[]) !!}" class="btn2-kit {!! Auth::user() && Auth::user()->isPremium()?'':'disabled' !!}" style="margin: 10px;"><i class="bs-add cu-search2"></i>Create Group</a>
        @else
        No results found
        @endif
    </div>
